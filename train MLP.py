...
def train(model, optimizer, criterion, lambda_):  # 1
    ...
    """
    h1_scrub_row, h1_scrub_col = scrub_index(rows=h1[0], cols=h1[1], raise_=1)
    h2_scrub_row, h2_scrub_col = scrub_index(rows=h2[0], cols=h2[1], raise_=1)
    h3_scrub_row, h3_scrub_col = scrub_index(rows=h3[0], cols=h3[1], raise_=1)
    h4_scrub_row, h4_scrub_col = scrub_index(rows=h4[0], cols=h4[1], raise_=1)

    h1_non_scrub_row, h1_non_scrub_col = non_scrub_index(rows=h1[0], cols=h1[1], raise_=1)
    h2_non_scrub_row, h2_non_scrub_col = non_scrub_index(rows=h2[0], cols=h2[1], raise_=1)
    h3_non_scrub_row, h3_non_scrub_col = non_scrub_index(rows=h3[0], cols=h3[1], raise_=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.06, metavar='LR', help='learning rate (default: 0.006)')
    parser.add_argument('--d', type=float, default=0, metavar='D', help='diagonal (default: 9)')
    parser.add_argument('--penalty_rate', '--wd', default=1e1, type=float, metavar='W', help='penalty rate (default: 1e1)')
